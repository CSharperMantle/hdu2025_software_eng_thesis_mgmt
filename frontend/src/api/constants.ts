import type { AssignmentStatus, ProgressOutcome, TopicReviewStatus, TopicType } from './models'
...
export const TOPIC_REVIEW_STATUSES = Array.from(TOPIC_REVIEW_STATUS_MAP.entries()).map(
  ([value, { name, color }]) => ({
    value,
    name,
    color,
...
export const ASSIGNMENT_STATUSES = Array.from(ASSIGNMENT_STATUS_MAP.entries()).map(
  ([value, name]) => ({
    value,
    name,
  }),
...
export const PROGRESS_OUTCOMES = Array.from(PROGRESS_OUTCOME_MAP.entries()).map(
  ([value, name]) => ({
    value,
    name,
  }),
